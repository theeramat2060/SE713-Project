...
async function main() {
    ...
    try {
        ...
        for (const admin of adminSeedData) {
            ...
            await client.query(
                'INSERT INTO "Admin" (username, password, created_at) VALUES ($1, $2, $3)',
                [admin.username, hashedPassword, admin.created_at]
            );
        }
        ...
        for (const constituency of constituencySeedData) {
            await client.query(
                'INSERT INTO "Constituency" (province, district_number, is_closed) VALUES ($1, $2, $3)',
                [constituency.province, constituency.district_number, constituency.is_closed]
            );
        }
        ...
        for (const party of partySeedData) {
            await client.query(
                'INSERT INTO "Party" (name, policy, logo_url, created_at) VALUES ($1, $2, $3, $4)',
                [party.name, party.policy, party.logo_url, party.created_at]
            );
        }
            ...
            if (userIds.length > 0 && candidateIds.length > 0) {
                ...
                await client.query(
                    `INSERT INTO "Vote" (user_id, candidate_id, created_at, updated_at) 
                     VALUES ($1, $2, $3, $4)`,
                    [userId, candidateId, vote.created_at, vote.updated_at]
                );
                votesInserted++;
            }
        }
